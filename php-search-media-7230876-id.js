...
/**
  ...
  const comparisonModal = document.querySelector('.program-comparison--modal .swiper-wrapper');
  
  const programListingTrigger = () => {
    
    

    // Get all the program listing items
    const programListingItems = document.querySelectorAll('.program-listing--item');  
    ...
    checkbox.addEventListener('change', (event) => {   
      ...
      if (event.target.checked) {  
        ...
        // Get the values from the parent article
        ...
        // Add the classes to the parent article
        // item.classList.add('program-listing--selected', `program--${name.toLowerCase().replace(/[’\s]+/g, '-')}`);
        item.classList.add('program-listing--selected', `program--${checkboxId}`);
  
        // Create a new div with the same classes as the parent article
        const newDiv = document.createElement('div');
        // newDiv.setAttribute('data-fancybox', 'comparison');
        // newDiv.classList.add('swiper-slide', 'text-margin-reset', 'program-listing--selected', `program--${name.toLowerCase().replace(/[’\s]+/g, '-')}`);
        newDiv.classList.add('swiper-slide', 'text-margin-reset', 'program-listing--selected', `program--${checkboxId}`);
  
        // Populate the new div with the harvested fields
        newDiv.innerHTML = `
        ...
        // Append the new div to the comparison modal
        comparisonModal.appendChild(newDiv);
        
        // Create a new button with the same classes as the new div
        const newButton = document.createElement('button');
        newButton.setAttribute('type', 'button');
        newButton.setAttribute('checkbox-id', checkboxId);
        // newButton.classList.add('program-listing--selected', `program--${name.toLowerCase().replace(/[’\s]+/g, '-')}`);
        ...
        // Insert the new button within the first .program-comparison--sticky-panel li
        const stickyPanel = document.querySelector('.program-comparison--sticky-panel');
        if (stickyPanel) {
        ...
        updateCheckboxes();     
          ...
          // const nameClass = `.program--` + `${name.toLowerCase().replace(/\s+/g, '-')}`;
          const nameClass = `.program--` + `${checkboxId}`;
          // Remove the classes from the parent article
          item.classList.remove('program-listing--selected', nameClass);
          
          // Remove the corresponding new div
          ...
          // Remove the corresponding new button
